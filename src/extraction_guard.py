...
def _is_valid_spacing(value):
    """
    Slab reinforcement spacing values almost always live between 50 mm
    and 500 mm in real drawings. Use this to discard OCR garbage like
    "5500 C/C" or "30 C/C" that surfaces when PDF text layers are corrupt.
    """
    try:
        n = int(value)
    except (TypeError, ValueError):
...
def _normalize_doubled(text):
    """
    pdfplumber sometimes returns each character doubled on rows that the PDF
    rendered with text shadowing (e.g. 'T8@150C/C' as 'TT88@@115500CC//CC').
    If every character in the string repeats in pairs, collapse them.
    """
    text = str(text)
    ...
    if all(text[i] == text[i + 1] for i in range(0, len(text), 2)):
        return text[::2]
    # Try a softer collapse on doubled digit runs followed by doubled letters.
    # Heuristic: if pairwise duplication holds for at least 80% of characters,
    # collapse.
    matches = sum(1 for i in range(0, len(text) - 1, 2) if text[i] == text[i + 1])
    total = len(text) // 2
    if total and matches / total >= 0.8:
        return text[::2]
    return text
...
def extract_rebar_parts(*texts):
    """
    Pull bar diameters (T<n>) and spacings (<n> C/C) out of free-form
    reinforcement strings. Handles the common slab-schedule notations:
      T12 @ 100 C/C       -> dia=T12, spacing=100 C/C
      T10 - 150 C/C       -> dia=T10, spacing=150 C/C
      T8-150              -> dia=T8,  spacing=150 C/C   (hyphen, no C/C)
      10T @ 150 c/c       -> dia=T10, spacing=150 C/C   (digit-then-T form)
      8T                  -> dia=T8
    """
    dia = []
    spacing = []
    for text in texts:
        for item in _as_list(text):
            raw = str(item).upper().replace("PHI", "T").replace("Ø", "T")
            # Token-by-token un-double for pdfplumber doubled-char garbage.
            tokens = raw.split()
            normalized_tokens = [_normalize_doubled(tok) for tok in tokens]
            clean = " ".join(normalized_tokens) if normalized_tokens else raw

            # Diameter — T-then-digit (canonical), e.g. T10
            for dm in re.finditer(r"\bT\s*(\d{1,2})\b", clean):
                value = f"T{dm.group(1)}"
                if value not in dia:
                    dia.append(value)

            # Diameter — digit-then-T form, e.g. 10T (pattern-4 / pattern-5)
            for dm in re.finditer(r"\b(\d{1,2})\s*T\b", clean):
                value = f"T{dm.group(1)}"
                if value not in dia:
                    dia.append(value)

            # Spacing followed by C/C in any form (with @ or - before it).
            for sm in re.finditer(r"(\d{2,4})\s*C\s*/?\s*C", clean):
                if not _is_valid_spacing(sm.group(1)):
                    continue
                value = f"{sm.group(1)} C/C"
                if value not in spacing:
                    spacing.append(value)

            # Spacing after @ without explicit C/C
            for sm in re.finditer(r"@\s*(\d{2,4})\b(?!\s*C)", clean):
                if not _is_valid_spacing(sm.group(1)):
                    continue
                value = f"{sm.group(1)} C/C"
                if value not in spacing:
            ...
            # Spacing in T<n>-<n> shorthand (pattern-8: T8-150)
            for sm in re.finditer(r"\bT\s*\d{1,2}\s*-\s*(\d{2,4})\b", clean):
                if not _is_valid_spacing(sm.group(1)):
                    continue
                value = f"{sm.group(1)} C/C"
                if value not in spacing:
            ...
            # Spacing in <n>T-<n> shorthand (rare, mirrors digit-T dia form)
            for sm in re.finditer(r"\b\d{1,2}\s*T\s*-\s*(\d{2,4})\b", clean):
                if not _is_valid_spacing(sm.group(1)):
                    continue
                value = f"{sm.group(1)} C/C"
                if value not in spacing:
